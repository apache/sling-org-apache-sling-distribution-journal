...
@Component(service = PackageHandler.class, name = "default")
public class DefaultPackageHandler implements PackageHandler {
    private static final Logger LOG = LoggerFactory.getLogger(DefaultPackageHandler.class);
    ...
    public void apply(ResourceResolver resolver, PackageMessage pkgMsg, ContentPackageExtractor extractor)
        ...
        switch (type) {
            case ADD:
                installAddPackage(resolver, pkgMsg, extractor);
                break;
            case DELETE:
    ...
    private void installAddPackage(ResourceResolver resolver, PackageMessage pkgMsg, ContentPackageExtractor extractor)
